...
public class runSimpleJob implements Callback {
	...
	public static void main(String[] args) {
		...
		try {
		  ...
		  // Pass the additional substitution
			...
			String inputsDir = job.getSubstitutedVariable("WPS_JOB_INPUTS_DIR");
			String outputsDir = job.getSubstitutedVariable("WPS_JOB_OUTPUTS_DIR");
			String auditsDir = job.getSubstitutedVariable("WPS_JOB_AUDITS_DIR");
			...
			java.net.URL inputURLList = new java.net.URL("http://storage.terradue.com/ssegrid/simpleJob/InputURLList.xml");
			...
			job.addCallback(Job.JOB_STATE, new runSimpleJob());
			job.addCallback(Job.JOB_STATEDETAIL, new runSimpleJob());
			// Let's GO!
			job.run();
			...
			if (job.getState() == State.DONE) {
				...
				java.util.Map simpleoutputMap = jsa.readComplexOutputs(job,
				...
				java.util.Map complexoutputXMLMap = jsa.readComplexOutputs(job,
